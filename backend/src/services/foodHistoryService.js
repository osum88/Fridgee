  ...
  for (const log of historyRaw) {
    ...
    if (!wasAddedToBatch) {
      ...
      currentBatch = {
        ...
        variant: log?.food?.variant?.title || "",
        user: user || null,
        price: Number(convertedPrice?.toFixed(2)) || 0,
        batchPrice: convertedPrice || 0,
        currency: currency,
        currentAmount: log?.instance?.amount || log.snapshotAmount || 0,
        snapshotUnit: log.snapshotUnit,
        snapshotAmount: log.snapshotAmount || 0,
        snapshotExpirationDate: log.snapshotExpirationDate || null,
        quantityBefore: log.quantityBefore || 0,
        quantityAfter: log.quantityAfter || 0,
