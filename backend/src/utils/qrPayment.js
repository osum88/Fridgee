...
export const generatePaymentFormat = ({countryCode, iban, amount, currency, message, name, bic = ""}) => {
...
export const isValidIbanOrBban = (input, countryCode) => {
    ...
    if (bbanRegex.test(cleaned)) {
        ...
        const [prefix, number] = accountPart.includes("-") ? accountPart.split("-") : ["0", accountPart];
