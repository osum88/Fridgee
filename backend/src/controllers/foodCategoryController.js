import { createFoodCategoryService, deleteFoodCategoryService, getFoodCategoriesByInventoryService, getFoodCategoryByIdService, updateFoodCategoryService } from "../services/foodCategoryService.js";
...
export const createFoodCategory = async (req, res, next) => {
    try {
        ...
        const newCategory = await createFoodCategoryService(Number(inventoryId), title);
...
export const getFoodCategoriesByInventory = async (req, res, next) => {
    try {
        ...
        const categories = await getFoodCategoriesByInventoryService(Number(inventoryId), userId, isAdmin);
...
export const updateFoodCategory = async (req, res, next) => {
    try {
        ...
        const updatedCategory = await updateFoodCategoryService(userId, Number(categoryId), title, isAdmin);
