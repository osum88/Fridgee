...
const validateForm = (errors, setErrors, inputText) => {
...
export default function EditInstanceScreen() {
  ...
  const handleSave = useCallback(() => {
    ...
    if (updating.current || isSubmitting || !validateForm(errors, setErrors, current)) return;
        ...
        onSuccess: async () => {
          queryClient.invalidateQueries({
           ...
           queryClient.refetchQueries({
            queryKey: ["food-detail", parseInt(activeInventory.id), parseInt(catalogId), parseInt(foodId)],
      ...
      >
        <ThemedView safe={true} style={styles.contentWrapper}>
          ...
          <Stepper
            ...
            onChange={(val) => updateFormValues(setInputText, "quantity", String(val))}
            min={1}
